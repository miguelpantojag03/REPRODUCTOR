...
export function RecentSongCard({ song }: { song: any }) {
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-square rounded-xl overflow-hidden bg-[#282828] mb-3 shadow-lg">
        <Image
      ...
      <p className={cn('text-sm font-semibold truncate leading-tight', isCurrent ? 'text-[#1db954]' : 'text-white')}>
        {song.name}
      </p>
      <Link
        href={`/artist/${encodeURIComponent(song.artist)}`}
        className="text-xs text-[#727272] hover:text-white transition-colors truncate block mt-0.5"
...
export function MostPlayedRow({ song, rank }: { song: any; rank: number }) {
  const { playTrack, currentTrack, isPlaying, togglePlayPause } = usePlayback();
  const isCurrent = currentTrack?.id === song.id;
  const rankColor = RANK_COLORS[rank] || '#727272';

  return (
    <div
      className={cn(
        'flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-150 cursor-pointer group',
        isCurrent ? 'bg-white/[0.08]' : 'hover:bg-white/[0.06] active:bg-white/[0.10]'
      )}
      onClick={() => isCurrent ? togglePlayPause() : playTrack(song)}
    >
      {/* Rank */}
      ...
      <div className="relative size-10 rounded-lg overflow-hidden bg-[#282828] flex-shrink-0">
        <Image src={getValidImageUrl(song.imageUrl)} alt={song.name} fill className="object-cover" sizes="40px" />
        {isCurrent && isPlaying && (
      ...
      <div className="flex-1 min-w-0">
        <p className={cn('text-sm font-medium truncate leading-tight', isCurrent ? 'text-[#1db954]' : 'text-white')}>
          {song.name}
        </p>
        <Link
          href={`/artist/${encodeURIComponent(song.artist)}`}
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        <div className="hidden sm:flex items-center gap-1 text-[11px] text-[#727272]">
          <TrendingUp className="size-3" />
          <span className="tabular-nums">{song.playCount}</span>
        </div>
        <span className="text-[11px] tabular-nums text-[#727272]">{formatDuration(song.duration)}</span>
      </div>
    </div>
  );
}
